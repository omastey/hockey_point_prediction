...
2. For breakout-classifier runs: add a row to `LEADERBOARD_BREAKOUT.md` with run #, model type, key changes, TP/FP/FN, precision, recall, ROC-AUC, avg precision, brier score, and notes
3. For PPG-regressor runs: add a row to `LEADERBOARD_PPG.md` with run #, date, model, key config, RMSE, MAE, R², train R², bias, per-position MAE, and notes
