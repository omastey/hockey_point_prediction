...
|---|---|---|---|---|---|---|
| **All metrics (tie)** | **#1 / #2** | XGBRegressor | **0.137** | **0.107** | **0.796** | MoneyPuck features added in run 2 didn't move the needle for PPG regression. Baseline still rules. |
...
| 2 | 2026-05-27 | XGBRegressor | Same config as Run #1. **MoneyPuck data joined into merged dataset** (xg, xg_flurry, oz_starts, on_ice_xg%, etc. + engineered `xg_luck`, `oz_start_pct`, `xg_per_60`). | 0.137 | 0.107 | 0.796 | 0.779 | -0.009 | 0.111 | 0.098 | Headline metrics unchanged — MoneyPuck features don't help the regressor at this config (top features still `ppg_league_percentile`, `players_ahead_on_team`, `ppg`). Train R² nudged up 0.005, suggesting the model is *very* slightly fitting xG noise but not generalizing from it. MoneyPuck signal lives in the breakout classifier instead (see runs 21+ in LEADERBOARD_BREAKOUT). |
...
| `ff3e107` | 2026-05-27 | MoneyPuck CSVs joined into merged dataset (xG, shot-quality, deployment); 4 engineered features added (`xg_luck`, `oz_start_pct`, `xg_per_60`, `xg_high_danger_per_60`). | New 4th data source. Boosts breakout classifier but flat for the regressor — recorded as Run #2. |
